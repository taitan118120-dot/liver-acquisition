...
# 2026-09-04: 0.35 → 0.40。Threads側の実測で、書き方を本音型・実務型に直した
# 代理店投稿は中央値55viewsとstory(52)を上回り、「代理店を増やすと沈む」前提が
# 崩れたため。同時に X の代理店棚も制度説明型8本を退役させて実務型に入れ替えた。
# 媒体ごとに比率をバラすと効果の切り分けができないので Threads と同じ40%に揃える。
AGENCY_POST_RATE = 0.40
