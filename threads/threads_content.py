...
確定ファクト（project_taitan_pro_note_facts）を厳守:
  還元率100%+α / 所属200名 / Pococha歴4年 / B帯月20-30万 /
...
LEAD_MAGNET_LINE = (
    ...
    "『ライバー新人期スタートダッシュガイド』を、下のリンクの友だち追加で無料配布中"
)

FACTS = """
【絶対に守る確定ファクト】
- 事務所名：TAITAN PRO
- 還元率100%+α。「90%」「相場70-85%」等は書かない。※「手数料なし」「手数料0円」という表現は使わない（還元率100%+αだけ書く）
- 所属ライバー数：Pococha・TikTok合わせて200名（内訳は書かない）
...
- 副業ペースの実例：1日4時間・週4日で配信した男性ライバーが2〜3か月で月20〜30万円（配信時間により変動、と必ず添える）
【禁止表現】
- 「手数料なし」「手数料0円」「手数料ゼロ」（還元率100%+αとだけ書く）
...
- 還元率100%+αが当たり前じゃない業界で、それが何を意味するか（手数料という言葉は使わない）
- 土日だけ・平日夜だけでも成立する理由（ノルマなし×時間ダイヤ。「毎日配信しなきゃ」は思い込み）
- 副業ペースの実例（FACTSの男性ライバーの実例。誇張せず「変動あり」を添えて誠実に）
- 会社バレが怖い人へ：バレる原因はほぼ住民税と身バレ。普通徴収など一般知識レベルの対策がある（断定せず「詳しくは専門家へ」）
...
- TAITAN PROは11の配信代理店と提携、還元率100%+α、所属200名という実体のある事務所だと伝える。
