...
■ 2026-09-04 代理店の増枠（30% → 40%）
  08-11に agency を「宣伝型 → storyと同じ本音型」に書き換えた後の実測
  （2026-08-11以降 n=40）: agency 平均55.0 / 中央値55（書き換え前は平均17.4）、
  story 平均72.9 / 中央値52。**中央値では agency が story を上回った**ので、
  「代理店を増やすと沈む」という前提はもう成立していない。
  あわせて TOPICS["agency"] を実務型（スカウト・立ち上げ・両立・トラブル）に寄せる。
  Noteの実測で代理店は実務型と募集型で月間PVが約9倍違ったため
  （[[feedback_agency_content_practical_not_recruiting]]）、増やす中身は実務側に寄せる。

3系統:
  - story  : 短い本音・現場のリアル（事務所名も誘導も入れない）。主力。
  - liver  : ライバー本人向けの気づき（役に立つ話→最後に小さく一言）
  - agency : 代理店パートナー向け（主軸のひとつ。募集要項ではなく実務を書く）
             LINE/代理店LPへの導線。書き方はstoryと同じ本音型に揃える。
...
# ── 2026-09-04 代理店をさらに厚くする（30% → 40%）────────────────
# 08-11の書き換え後の実測（data/threads_insights.csv, 2026-08-11以降 n=40）:
#   story 平均72.9 / 中央値52（n=23）
#   agency 平均55.0 / 中央値55（n=13）  ← 書き換え前は平均17.4
#   liver 平均101.3 / 中央値61（n=3・母数が小さいので参考値）
# 中央値では agency が story を上回っている。「代理店を増やすとリーチが沈む」は
# 書き方を直した時点で解消済みなので、比率をもう一段上げる。
# 増やす分は今回も liver から取る（story 50% は死守）。
TARGET_MIX = {"story": 0.50, "liver": 0.10, "agency": 0.40}
AGENCY_MAX_SHARE = 0.45   # 代理店は多くても45%まで
...
できれば「実際に手を動かしている場面」を書く（声をかける・初配信に付き添う・
返信が来ない・辞めそうな子に連絡する）。「代理店とは」「メリット」「なる方法」
「稼げるのか」といった制度の説明は、読まれないことが実測で分かっているので書かない。
...
# （2026-08-10 の並行セッションが旧特典PDF名の広域パターンをここに足していたが、
#   それも媒体共通のルールなので facts_patterns.COMMON_NG_PATTERNS に統合済み）
THREADS_NG_PATTERNS = []
