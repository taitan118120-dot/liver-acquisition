...
_DEFAULT_AGENCY_TEMPLATE_CAST = (
    ...
    "🎯昼間の隙間時間で月20万以上の継続収益\n"
    "🎯水商売の人脈ですぐスカウトできる\n"
    "🎯ご自身がライバーになるのもアリ（達成条件に応じた時給上乗せ報酬あり）\n"
    "🎯完全在宅・スマホ完結\n\n"
    "🎙ラジオライバー可能\n\n"
    "「興味あり」とご返信いただければ詳細お送りします💌\n\n"
    "→ https://taitan-pro-lp.netlify.app/#apply"
...
def init_db():
    with get_conn() as conn:
        ...
        try:
            # 2026-08-01 追加: S帯月収の具体数字は確定ファクト表で全面禁止のため定性表現へ。
            # 旧DBには「月収100万円超…／最高月収600万円以上(Pococha S6帯)」が残っている。
            HEADCOUNT_RULES = [
                ("総勢150名所属", "総勢200名所属"),
                (
                ...
                # 上の偽実績「90日で月収100万円超」の元ネタでもある。x_app/db.py にも同じルールあり。
                (
            ...
            # 上の literal ルールは既知の2形だけを潰す。設定UIからの手編集で
            # 「最大5500円/h」のような別の数字が入っている可能性があるので、
            # 金額部分をワイルドカードにした掃き出しも併せて掛ける（旧値でなくフィールドで当てる）。
            HOURLY_CAP_PATTERNS = [
                # 「（時給上乗せ最大5000円/h）」のような丸ごと括弧の言い回し
                (re.compile(r"[（(]\s*時給上乗せ\s*最大\s*[0-9,]+\s*円\s*/\s*[hH]\s*[）)]"),
                 "（達成条件に応じた時給上乗せ報酬あり）"),
                # 「時給上乗せ報酬（最大5,000円/h）」のように後置きの括弧だけ数字を持つ形
        ...
        # 追記される。その後に文言マイグレーションが走ると同一文が二重化するので、
        # 最後に順序保持で重複を潰す。冪等。
        try:
            tpl_row = conn.execute("SELECT value FROM settings WHERE key='templates'").fetchone()
            if tpl_row:
                ...
                if isinstance(templates, dict):
                    for k, v in templates.items():
                        ...
                        deduped = []
                        for tpl in v:
                            key = tpl if isinstance(tpl, str) else json.dumps(tpl, ensure_ascii=False)
                            if key in seen:
                                changed = True
                                continue
                            seen.add(key)
