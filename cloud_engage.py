...
2026-09-16: 定期実行していた `auto_engage.yml` は廃止・削除した（GitHub上では
2026-04-04以降 disabled_manually で、cron は一度も発火していなかった）。
このファイルを消さないこと —— `x_reply_digest.py` が `is_ng` を import しており、
いまはリプ候補のNG判定ライブラリとして生きている。検索部分は手動実行用に残しているだけ。
"""
