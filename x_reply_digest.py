...
OFF_TOPIC_WORDS = [
    ...
    # 店名しか書かないパチンコ実戦配信が素通りするので、業界語でも落とす
    "出玉", "初当り", "初当たり", "万発", "実戦", "甘デジ", "等価", "ホール",
    "uber", "ウーバー", "出前館", "配達員",
...
AD_WORDS = [
    ...
    "dmください", "dm下さい", "dmお待ち", "募集中です", "メンバー募集",
]
...
LISTENER_WORDS = [
    ...
    "記念配信", "凸待ち", "アーカイブ",
    # 相手の配信に行く側の言い回し。「初配信」に反応して混ざってくるので落とす。
    "会いに行く", "見に行く", "行くね", "覗きに", "してくれる", "してくれた", "神対応",
]
    ...
    for q in queries:
        ...
        for t, u in found:
            ...
            if mode == "empathy":
                ...
                # 共感枠は「自分のことを書いている人」限定。リスナー語が1つでも出たら落とす
                # （手で1件ずつリプするので、件数より精度が要る）。
                if intent == 0 or listener >= 1:
