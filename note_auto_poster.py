...
def _acquire_csrf_token(session, verbose=True):
    ...
    したがって本関数は「取れたら付ける」以上の意味を持たない。呼ばれるのは api_login の
    Playwright UIログイン経路（ローカル専用のフォールバック）だけで、CI の Cookie 経路
    （_http_full_post）は通らない。note が将来 CSRF を復活させたときのために残してある
    が、取得できなくても異常ではない。
    """
