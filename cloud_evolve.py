...
- 「利用者の約40%は20代」のように**割合が主語の後ろに来る形**も同じく禁止
  （「残りの88%は」「上位層の43%は」も同じ。自分で集計した事務所の内部データでも書かない）。
- 「代理店ビジネスの9割は〜」のように**割合がそのまま述語につながる形**も同じく禁止。
- 「30代は20代より20%高い」のように、**別のものと比べた増減を割合で書く形**も同じく禁止
  （「◯%増」「◯%多い」「◯%近く低い」も同じ）。比べたいときは割合を出さず
  「◯◯のほうが伸びやすい」と傾向だけを書く。
- 割合の代わりに、断定したいときは「多い」「よく見る」「ほとんど」で書く。

【金額の下限（違反が最も多い。必ず守る）】
- 収入は 3ヶ月15〜20万 / 6ヶ月30〜40万 / Pococha B帯 月20〜30万 のみ。
- **「月◯万」と書くとき、◯が15未満なら一律で違反**（「月3万」「月10万」「数万円」「お小遣い程度」型）。
...
- 「オフの日」（月4日の強制休配信日）は **Pocochaの制度**。TAITAN PRO・事務所側の制度として書かない
- 他の事務所を下げる書き方をしない（「多くの事務所は〜」「一般的な事務所と違って〜」等）
- 「絶対稼げる」「確実に」「必ず月◯万」「保証」等の断定・保証表現は使わない
- 「不労所得」「権利収入」は使わない（マルチ的表現）。「働かなくても入ってくる」
  「寝てても稼げる」のような**言い換え**も同じ扱いで禁止
- 「多数輩出」「多くの実績」「続々と」「数百人」「何百人」「数千」「何千」など、
  裏の取れない実績の誇張は書かない
- 「カーブアウト（パートナー）」という呼称は使わない。名乗るなら TAITAN PRO
- 「京都コレクション」は**実在しないイベント**。他の出演機会と並べて書かない
- 「上位10%」のような順位の割合、「挫折率」という語も出典が無いので書かない
...
- 業界の市場規模・成長率を裏取りなく書かない。率（「市場規模は毎年130%成長」）だけでなく
  **金額（「国内1,500億円規模」「グローバル7兆円」）と国別順位（「日本は世界で3番目」）も同じ**。
  市場の話をしたいときは規模を出さず、伸びている理由（5Gの普及・投げ銭文化の定着・
  企業のライブコマース参入）だけを書く
- 代理店の費用条件はLPが正本（加盟金・保証金・月会費・ロイヤリティの前払いなし）。
  「ロイヤリティの仕組みあり」のようなLPと食い違う書き方をしない
"""
...
FACTS_COVERAGE = {
    ...
    # 2026-09-04: 上のラベルは「違約金」「契約期間」の語が出たかしか見ないので
    # 記事・IGでは WARN に落としてある。完成形の自社主張だけを別ラベルにして
    # どの媒体でも赤のまま出す（投稿済みIG 5本で実測）
    "「いつでも退所」「違約金なし」の完成形（自社主張）": "違約金なし",
    # 2026-09-04: 「150人以上のライバーが所属」のように**数字が先に来る形**を
    # 既存の `所属◯名` パターンが取りこぼしていた（投稿済みIG 10本で実測）
    "所属数が200名以外（数字が先に来る形）": "200名",
    ...
    "出典なしの「上位N%」": "上位10%",
    "出典なしの挫折率": "挫折率",
    "所属数が200名以外（合計表記）": "200名",
    "特典PDF名は『ライバー新人期スタートダッシュガイド』": "Pococha新人期スタートダッシュ",
    "リンクが lit.link（公式LINEでない）": "lit.link",
    "リスナーの呼び捨て": "リスナーさん",
    "断定・保証表現": "保証",
    "マルチ的表現（不労所得の言い換えを含む）": "不労所得",
    "根拠なしの実績誇張": "多数輩出",
    "少額表記（数万円/お小遣い程度）": "数万円",
    "「オフの日」はPocochaの制度。事務所の制度として書かない": "オフの日",
    "他社を下げる書き方": "他の事務所",
    "根拠のない市場規模・成長率": "市場規模",
    "代理店の費用条件がLPと食い違う恐れ（ロイヤリティ）": "ロイヤリティ",
    # facts_patterns.py（媒体共通の正本）由来。ラベルは実際に検品を通して集める
    "出典なしの割合統計（離脱/成功率）": "割合",
    "出典なしの割合統計（割合が主語を修飾）": "割合",
    "出典なしの割合統計（割合がそのまま述語）": "割合",
    "出典なしの割合統計（他と比べた増減）": "20%高い",
    "出典なしの割合統計（割合が主語の後ろ）": "利用者の約40%は",
...
_SHARED_PROBES = [
    ...
    "9割のライバーはフリーで十分",
    "9割は正直そんなもんだと思う",
    "30代の中央値は20代より約20%も高い",
    "利用者の約40%は25〜35歳という結果",
    "月3万くらいから始まる",
