"""
Instagramバズ特化カルーセル生成（2026-07-11 全面刷新 / 2026-09-11 実測で検証）

旧 ig_content_generator（ブログ要約×1枚絵）から、バズ狙いの縦型カルーセル
（4:5・表紙フック＋1スライド1メッセージ）へ全面転換したもの。

【2026-09-11 実測】転換の根拠だった「旧単画像はリーチ0」は誤り。
測定データが無い状態で書かれた推測だった（当時 data/ig_insights.csv は全指標0）。
92本の実測（`python instagram/ig_insights.py --report-only`）はこう出ている:

  中央リーチ  単画像 11.0（n=63） / カルーセル 10.0（n=25）
  自動生成のみ blog 9.0 ＜ viral(カルーセル) 10.0 ＜ twitter 13.0
  フォロワー15人。92本中73本(79%)がリーチ15以下＝フォロワーの外に出ていない
  カルーセル25本のリーチ最大値は14。一度も外に出ていない
  92本の合計フォロー獲得 = 1

つまり **フォーマットはリーチを動かしていない**。単画像期(4〜7月)とカルーセル期
(7月〜)は時期が完全に分かれていて重なりが1本しか無いので、そもそも
「単画像 vs カルーセル」の比較になっていない。「平均リーチ 単画像21.3 vs
カルーセル10.1」は、views < reach の計測異常4本が平均を持ち上げていただけ。

→ 結論: カルーセルは据え置き（巻き戻す根拠が無い）。ボトルネックは
   フォーマットではなく配信面（15フォロワー・非フォロワーに出ない）なので、
   ここをいじっても測定可能な差は出ない。詳細は memory の
   project_ig_viral_carousel / project_ig_insights_baseline。
...
- 所属のむうくん（20歳・大学生・栃木県、2026年4月デビュー）は週4日・1日平均4時間の配信で、デビューから約3ヶ月でS帯のトップライバーに到達。配信月収50万円超
...
VIRAL_THEMES = [
    ...
    {"id": "muukun_story", "type": "story", "tag": "所属ライバーの実話", "use_facts": True,
     "title": "20歳大学生が3ヶ月でS帯まで行った話",
     "angle": "栃木在住の普通の大学生むうくんの実話。週4日・1日平均4時間という現実的なペースを積み重ねて約3ヶ月でS帯のトップライバーに到達した過程を、誇張せずリアルに"},
