...
- 「手数料なし」「手数料0円」「手数料ゼロ」は書かない。報酬の話は「還元率100%」とだけ書く。
...
- 「DMで詳細」「DMください」「LINEで連絡」「公式LINE」など外部誘導/DM誘導の文言は完全禁止。
  CTAは固定文の「プロフィールから」のみ。例外なし。
- 「無料」「無料相談」を1キャプション内で2回以上連呼しない（CTAの1回のみ）。
- 「最短」「即日」「一瞬で」「誰でも」「絶対」「保証」「確実」など断定的・即効性を匂わす語は避ける。
- 「稼げる」より「自分らしく働ける」「収入の選択肢が増える」など柔らかい表現を優先する。
- ハッシュタグの大ボリュームに「#副業」「#副業女子」を両方入れない（どちらか1つ）。
"""
...
def _polish_caption(text):
    ...
    if len(tags) < 10:
        ...
        pool_large = [
            "#ライバー", "#ライブ配信", "#Pococha", "#TikTokLIVE",
            "#在宅ワーク", "#フリーランス",
        ]
        pool_medium = [
            "#ポコチャ", "#ライバー事務所", "#ライバー募集", "#TikTokライバー",
            "#ポコチャライバー", "#ライバーになりたい", "#自分らしく働く",
            "#スマホで稼ぐ", "#おうち時間", "#新しい働き方",
        ]
        pool_small = [
            "#タイタンプロ", "#ライバーデビュー", "#ポコチャ攻略",
