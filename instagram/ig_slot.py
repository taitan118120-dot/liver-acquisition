...
■ 狙う時間帯の根拠（2026-09-11 実測。結論＝時刻では決まらない）
2026-09-11 に instagram_manage_insights を付与して data/ig_insights.csv を
実データで再生成した（92本・2026-04〜09）。時刻別リーチの中央値はこうなった:

   0時 20.0(n=6)   1時 9.5(n=8)   2時 10.5(n=2)   6時 9(n=1)
   8時 13(n=7)     9時 9.5(n=8)  12時 15(n=1)   15時 10(n=7)
  16時 9(n=3)     20時 10(n=13)  21時 11(n=15)  22時 14(n=11)  23時 10.5(n=10)

窓内(19:50-23:40) n=49 中央11 ／ 窓外 n=43 中央11 で **差が無い**。
期間を4〜6月だけ・7月以降だけに切っても同じで、どの時刻も中央9〜20に収まる。
Threads では時刻で中央値が3倍違った（threads/threads_slot.py）が、
**IGでは時刻はリーチを説明していない**。リーチの絶対値が小さすぎて
（全期間の平均18.3・中央11）、時刻差がノイズに埋もれているため。

よって WINDOW は 19:50-23:40 のまま据え置く。根拠は「実測でここが最良」では
なく次の2点だけ:
  (1) どの時刻でも差が無いなら、深夜3〜6時台に出しにいく理由も無い
  (2) schedule遅延の実測最大間隔163分を吸収できる幅が要る（下の WINDOW 参照）
リーチが増えて時刻差が見えるようになったら測り直すこと:
  python instagram/ig_insights.py --slots-only
（トークンが失効/権限不足になっていたら instagram/TOKEN_REISSUE.md）

■ 時刻より先に効く変数（同じ実測より）
  フォーマット: 単画像 n=67 平均リーチ21.3 ／ カルーセル n=25 平均リーチ10.1
  生成ソース  : blog 24.6 ＞ twitter 14.8 ＞ viral(カルーセル) 10.1
  月別平均リーチ: 4月20.0 → 5月21.6 → 6月26.3 → 7月12.9 → 8月9.7 → 9月9.5
2026-07-11 の「バズ特化カルーセルへの転換」（[[project_ig_viral_carousel]]）と
リーチ半減の時期が一致している。窓をいじるより先にここを見直すこと。
