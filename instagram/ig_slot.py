...
# 2026-09-11 に cron を30分おきにして実測したところ、GitHub は30分ごとに
# 起動してくれるわけではなく、**scheduleランの実配信は3時間半に1回程度**だった
# （IG: cron投入から最初のランまで3時間25分、Threadsも同時刻帯で3時間41分に1回）。
# 遅延したランがキューに溜まると後続の schedule イベントが捨てられるため。
# つまり窓を2時間半にすると、その日どのランも窓に入らず0本になる日が
# 4日に1日ほど出る計算になる。
