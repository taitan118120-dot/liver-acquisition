/* ==========================================================================
   ...
   ■ GA4について（2026-08-10 ユーザー決定：**GA4は使わない**）
     GA4_ID は空のまま運用する。空でも Google広告のコンバージョン計測は従来どおり動く。
     ただし **GA4が無い間、line_cta_click と流入元は「送っているが受け取り手がいない」状態**で、
     どこにも蓄積されない。どのボタンが押されたか・どの媒体から来たかは見られない。
     方針が変わったら、下の GA4_ID に測定ID（G-…）を入れて push するだけで全LPが即有効になる。
     GA4以外の受け皿（GTMコンテナ等）を入れる場合も dataLayer には既に流れている。
