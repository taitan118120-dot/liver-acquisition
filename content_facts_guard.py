...
IG_POSTED_WARN_LABELS = frozenset({
    ...
    # 2026-09-05: この中で唯一 ig_auto_013 のタイトル「【ライバーの6割が悩む
    # 辞めたい病の特効薬】」だけは離脱率の具体的な主張に読めるとユーザーに
    # 指摘してもらい、そこだけ直した（ig_facts_fix_20260905.py）。
    # 残り14箇所はクリックベイトのフックのままなので、カテゴリごとWARNに残す
    # （個別に外すとまた同じ「9割が知らない」を毎回人が読む羽目になる）。
    "出典なしの割合統計（割合がそのまま述語）",
