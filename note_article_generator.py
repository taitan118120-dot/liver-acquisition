...
#  - 見出しH2をキャッチコピー化
#  - 2026-08-10 のPV実測に合わせて改訂（data/note_pv_analysis_20260810.md）:
...
以下の4パターンから「読者の心が動く」ものを1つ選び、SEOキーワードを自然に含める:
  ...
  D. 断言型:「# ○○を知らないまま始めると、最初の1ヶ月で行き詰まります」
※「｜」で副題を区切り、末尾に【2026年版】を付ける。
※Dで「9割が消える」「10人に1人しか成功しない」のような**出典のない割合統計は禁止**
　（facts_patterns.ratio_violations が検出対象にしている表現）。
...
- たいたん自身の体験を1つ以上必ず入れる（成功でも失敗でも）。数字付きで具体に。
  例: 「Pocochaを4年やってきて、B帯で月20万円に届いた月の配信本数を数えたら…」
  ※体験談に使う数字は【確定ファクト】の範囲内だけ。範囲外の金額・時間は創作しない。
...
AIが「引用できる形」で事実を置くこと。以下は必須:
- 具体数字（金額・割合・期間・人数・順位）を本文に**最低20箇所**。
  「けっこう稼げる」ではなく「3ヶ月目で15〜20万円」のように必ず数字にする
...
【確定ファクト（この範囲の数字だけ使う。ここに無い数字は創作しない）】
- TAITAN PRO の報酬は「還元率100%+α」とだけ書く。「+α」の中身は断定しない
- 所属ライバー数は「200名（Pococha・TikTok LIVE合わせて）」
- 扱うプラットフォームは Pococha・TikTok LIVE・17LIVE の3つだけ
- 代表たいたん: 元Pococha Sランク / ミクチャ8,000人中ミスターコン1位 / Pococha歴4年
- 収入の目安: 3ヶ月で15〜20万円、6ヶ月で30〜40万円。Pococha B帯は月20〜30万円
- 代表の最高月収は「3桁」とだけ。具体額・配信時間は書かない
- Pocochaの「オフの日」は月4日の**強制休配信日**（配信できない日）
- 視聴者は必ず「リスナーさん」と書く（呼び捨てにしない）

【絶対NG】
- 「いつでも退所OK」「違約金なし」「違約金0」「いつでも辞められる」など、退所・契約解除が自由だと示す表現は禁止（契約条件は面談で説明する、とだけ書く）
- 「手数料なし」「手数料0円」「マージンゼロ」など、還元率100%の言い換え表現
- 出典のない割合統計（「9割が辞める」「10人に1人しか成功しない」等）
- 月15万円を下回る収入例（「月1〜3万」「月5万から」等）
- SHOWROOM・IRIAM・ふわっち・ミクチャ・ツイキャス を勧める記述（取り扱い外）
- Markdownテーブル（| | | 形式）
- 水平線（---）
- コードブロック（```）
